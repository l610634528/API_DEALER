...
class TestSpecialPrice(unittest.TestCase):
    ...
    def get_update_date(self):
        ...
        res = requests.post(url=self.url_finish, data=submit_date, headers=self.headers)
        res_text = json.loads(res.text)
        for i in range(0, 100):
            employeeId = int(res_text['data']['result']['records'][i]['employeeId'])
            if employeeId == agent_id:
                uniqueId = res_text['data']['result']['records'][i]['uniqueId']
                # print(uniqueId)
                return uniqueId

    def get_update_detail(self):
        uniqueId = self.get_update_date()
        submit_date = json.dumps(
            {"userName": self.name,
             "userId": self.userid,
             "token": self.token,
        ...
        res_text = json.loads(res.text)
        type = int(res_text['data']['result']['priceApplyDetail']['type'])
        projectName = res_text['data']['result']['priceApplyDetail']['projectName']
        projectAddress = res_text['data']['result']['priceApplyDetail']['projectAddress']
        startTime = res_text['data']['result']['priceApplyDetail']['startTime']
        endTime = res_text['data']['result']['priceApplyDetail']['endTime']
        monthly = res_text['data']['result']['priceApplyDetail']['monthly']
        monthlyOther = res_text['data']['result']['priceApplyDetail']['monthlyOther']
        annual = res_text['data']['result']['priceApplyDetail']['annual']
        annualOther = res_text['data']['result']['priceApplyDetail']['annualOther']
        rebate = res_text['data']['result']['priceApplyDetail']['rebate']
        rebateOther = res_text['data']['result']['priceApplyDetail']['rebateOther']
        other = res_text['data']['result']['priceApplyDetail']['other']
        applyReason = res_text['data']['result']['priceApplyDetail']['applyReason']
        ...
        for i in range(0, length_product):
            ...
            price.append(res_text['data']['result']['productList'][i]['price'])
        pid = []
        agentType = []
        orgName = []
        agentAbbr = []
        ...
        return uniqueId, type, projectName, projectAddress, startTime, endTime, monthly, monthlyOther, annual, \
               annualOther, rebate, rebateOther, other, applyReason, id_product, createTime, updateTime, idStr, praId, \
               productId, signProductId, productName, productDetail, productColor, price, pid, agentType, orgName, agentAbbr, \
               agentCode, companyName, branchName, branchCode, number, orgCode, id_agent

    # 修改
    def test_i_updateDetail(self):
        agent_id = self.agent_id
        uniqueId, type, projectName, projectAddress, startTime, endTime, monthly, monthlyOther, annual, \
        annualOther, rebate, rebateOther, other, applyReason, id_product, createTime, updateTime, idStr, praId, \
        productId, signProductId, productName, productDetail, productColor, price, pid, agentType, orgName, agentAbbr, \
        agentCode, companyName, branchName, branchCode, number, orgCode, id_agent = self.get_update_detail()
        data_update = json.dumps(
            {"keys": [],
             ...
             "annual": annual,
             "annualOther": annualOther,
             "rebate": rebate,
             "rebateOther": rebateOther,
             "other": other,
