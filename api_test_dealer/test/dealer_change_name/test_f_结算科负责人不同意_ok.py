...
class TestChangeName(unittest.TestCase):
    ...
    # 结算科新账户名称保存后提交
    def test_c_first_trail_save(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 保存新名称后初审审批同意
    def test_d_first_trail_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 结算科负责人审批不同意
    def test_g_principal_disagree(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 结算科初审再次同意
    def test_h_first_trail_aganin_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_first_trail = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 结算科复审再次同意
    def test_i_recheck_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_recheck = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 市场部领导审批再次同意
    def test_j_market_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_market = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 结算科负责人审批再次同意
    def test_k_principal_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_principal = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 城市零售申请表提交图片
    def test_l_city_upload_image(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 统计科审核同意
    def test_m_statistics_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
