...
class TestChangeName(unittest.TestCase):
    ...
    # 初审不同意
    def test_c_first_trail_disagree(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # # 获取更新信息页面
    # def test_e_detailapp(self):
    #     data_detailapp = json.dumps({
    #         "id":self.userid,
    #         "token":self.token
    #         })
    #     res = requests.post(url=self.url_detail, data=data_detailapp, headers=self.headers)

    # 结算科新账户名称保存后提交
    def test_e_first_trail_save(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 保存新名称后审批再次同意
    def test_f_first_trail_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_first_trail = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 城市零售申请表提交图片
    def test_j_city_upload_image(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 统计科审核同意
    def test_k_statistics_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
