...
class TestChangeName(unittest.TestCase):
    ...
    # 城市零售申请表提交图片
    def test_h_city_upload_image_start(self):
        pid, tid, employ_id = self.get_pid_tid()
    ...
    # 初审再次审批同意
    def test_i_first_trail_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_first_trail = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 市场部领导再次审批同意
    def test_k_market_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_market = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 结算科负责人再次审批同意
    def test_l_principal_again_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_principal = json.dumps({
            "actType": "rename",
            "userId": self.userid,
    ...
    # 城市零售申请表提交图片
    def test_m_city_upload_image(self):
        pid, tid, employ_id = self.get_pid_tid()
        data_upload = json.dumps({
            "actType": "rename",
            "userId": self.userid,
        ...
        # 统计科审核同意
    def test_n_statistics_agree(self):
        pid, tid, employ_id = self.get_pid_tid()
