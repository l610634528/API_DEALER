...
class TestBaseSave(unittest.TestCase):
    ...
    def test_f_复审同意(self):
        ...
        res = requests.post(url=self.url_audit, data=data_settle_recheck, headers=self.headers)
    
    # 市场部审核不同意
    def test_g_市场部不同意(self):
        # 获取tid3
        ...
        data_bazaar = json.dumps({
            ...
            "var": {
                ...
                "values": "false,不同意_3市场部不同意",
                "types": "B,S"
                },
            "token": self.token
            })
        res = requests.post(url=self.url_audit, data=data_bazaar, headers=self.headers)

    # 结算科再初审同意
    def test_h_初审再次同意(self):
        tid_1_1 = self.tid_1_first_trial()
        data_h_first_trial_2 = json.dumps({
            "actType": "sign",
            "userId": self.userid,
            "pId": self.pid_first_trial(),
            "taskId": tid_1_1,
        ...
        res = requests.post(url=self.url_audit, data=data_h_first_trial_2, headers=self.headers)

    # 结算科再复审同意
    def test_i_复审再次同意(self):
        # 获取tid2
        tid_2 = self.tid_1_first_trial()
        data_settle_recheck = json.dumps({
            "actType": "sign",
