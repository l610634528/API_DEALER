...
class TestBaseSave(unittest.TestCase):
    ...
    # 结算科负责人再同意
    def test_j_负责人再次同意(self):
        tid_6 = self.tid_1_first_trial()
        data_finish_2 = json.dumps({
            "actType": "sign",
            "userId": self.userid,
            "pId": self.pid_first_trial(),
            "taskId": tid_6,
        ...
        res = requests.post(url=self.url_audit, data=data_finish_2, headers=self.headers)

    # 总监1审核再同意
    def test_k_总监1再次同意(self):
        tid_7 = self.tid_1_first_trial()
        data_majordomo_2_1 = json.dumps({
            "actType": "sign",
            "userId": self.userid,
            "pId": self.pid_first_trial(), 
            "taskId": tid_7,
        ...
        res = requests.post(url=self.url_audit, data=data_majordomo_2_1, headers=self.headers)
