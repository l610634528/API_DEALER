...
class TestAdditional(unittest.TestCase):
    ...
    def test_b_提交后状态判断(self):
        ...
        res = requests.post(url=self.url_info, data=data_info, headers=self.headers)
        res_text = json.loads(res.text)
        res_agentstatus = res_text['data']['result']['records'][0]['agentStatusText']
        try:
            self.assertEqual(res_agentstatus, '补充备案审核中')
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 补充备案获取pid
    url_list_task = dealer_url + '/act/list/task'
    def get_task_pid_tid(self):
        data_pid = json.dumps({
            "userId": self.userid,
            "page": 1,
            "size": 10,
    ...
    def test_c_市场部不同意(self):
        ...
        res_employid = json.loads(res.text)
        res_employid_text = res_employid['data']['result']['employeeId']
        ...
        res_text_reason = res_text['data']['result']['records'][0]['reasonType']
        try:
            self.assertEqual(res_text_reason, 'false') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 驳回后取employeeid
    url_finish_employeeid = dealer_url + '/act/list/user/finished'
    def get_finish_employeeid(self):
        data_finish_employeeid = json.dumps({
            "userId": self.userid,
            "page": 1,
            "size": 10,
            "actType": "supplement",
        ...
        res = requests.post(url=self.url_finish_employeeid, data=data_finish_employeeid, headers=self.headers)
        res_employid = json.loads(res.text)
        res_employid_id = res_employid['data']['result']['records'][0]['employeeId']
        return res_employid_id  #1843
        
    # 驳回后取id
    def get_query_id(self):
        agentid = self.get_finish_employeeid()
        data_query_id = json.dumps({
            "agentId": agentid,
            "justLook": "0",
            "token": self.token
            })
        res = requests.post(url=self.url_query, data=data_query_id, headers=self.headers)
        res_text = json.loads(res.text)
        res_id = res_text['data']['result']['reciverAddressList'][0]['id'] #3447
        return res_id
    # 再次提交补充备案
    def test_d_补充备案再次提交(self):
        employid = self.get_finish_employeeid()
        id_d = self.get_query_id()
        data_apply = json.dumps({
            "userId": self.userid,
            "userName": "吕结算科", 
            "agentId": employid,
    ...
    def test_e_市场部再次同意(self):
        ...
        res_finish = requests.post(url=self.url_finish, data=data_finish, headers=self.headers)
        res_text = json.loads(res_finish.text)
        res_text_name = res_text['data']['result']['records'][0]['task']['name']
        try:
            self.assertEqual(res_text_name, '结算科操作员') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 结算科操作员初审同意
    def test_f_初审同意(self):
        employid, pid, tid = self.get_task_pid_tid()
        ...
        try:
            self.assertEqual(res_task_text, '结算科复核') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 结算科复核同意
    def test_g_复审同意(self):
        employid, pid, tid = self.get_task_pid_tid()
        ...
        res_task_text = str(res_text['data']['result']['records'][0]['task']['name'])
        try:
            self.assertEqual(res_task_text, '市场领导') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 市场部领导同意
    def test_h_市场部领导同意(self):
        employid, pid, tid = self.get_task_pid_tid()
        ...
        res_task_text = str(res_text['data']['result']['records'][0]['task']['name'])
        try:
            self.assertEqual(res_task_text, '结算科负责人') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 结算科负责人同意
    def test_i_负责人同意(self):
        employid, pid, tid = self.get_task_pid_tid()
        ...
        res_task_text = str(res_text['data']['result']['records'][0]['task']['name'])
        try:
            self.assertEqual(res_task_text, '总监2') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))

    # 总监2审核同意
    def test_j_总监2同意(self):
        employid, pid, tid = self.get_task_pid_tid()
        ...
        try:
            self.assertEqual(res_processType, '1') and self.assertEqual(res_employid_text, employid)
        except Exception as e:
            print('Assertion test fail.', format(e))
