...
class TestBaseSave(unittest.TestCase):
    ...
    #     data_tid_1 = json.dumps({
    #         "pId": self.pid_first_trial(),
    #         "actType": "sign",
    #         "token": self.token
    #         })
    #     res = requests.post(url=self.url_pid, data=data_tid_1, headers=self.headers)
    #     restext = json.loads(res.text)
    #     tid = restext['data']['result']['records'][0]['task']['id']
    #     return int(tid)
